...
These are derived from `LEAGUE_FORMAT` / `tep_tier` config (`off` | `te+` | `te++`). Default is `te+`. When `tep_tier=off`, the `tep` key is omitted entirely (do not send empty `tep=`).
