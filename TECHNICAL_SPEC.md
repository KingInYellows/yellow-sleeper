...
* **League-true query (2026-09-21):** Parse `league_format` into documented `/values/current` params ([FantasyCalc API Docs](https://fantasycalc.com/api-docs)): `numTeams` ∈ {8,10,12,14}, `numQbs` `"1"`|`"2"`, `ppr` `0`|`0.5`|`1`, `tep` omitted|`te+`|`te++`. Do not send `tep=none`. Do not invent multipliers. Default `14-team SF PPR 0.5 TEP` still sends `tep=te+`. Unparseable or undocumented combinations skip the HTTP fetch; health shows `fantasycalc_values` missing; tools return missing/partial with reasons. Never serve a different query’s board as if the format matched. Cache variant remains `v1` + sorted params of the query that was sent.
* **Provider-backed picks (2026-09-21):** Index FantasyCalc rows with `position=PICK`. Match generic names `{season} {1st|2nd|3rd|4th|5th}` only (PR #15 generic-row idea, re-implemented). Do not use Sleeper `roster_id` as a draft slot and do not auto-select Early/Mid/Late bands. Fallback remains `PICK_VALUE_BY_ROUND` as `config_pick_table`, labeled not-provider.
...
* **CSV overlay (2026-09-21):** Optional `values_overlay_path` CSV keyed by Sleeper player id. Contract source name stays `xlsx`. Overlay wins over FantasyCalc when the file loads. A missing configured file is an explicit status, not a silent FantasyCalc fallback labeled as overlay. Default unset path is unchanged. Cache identity includes `overlay-on` only when the overlay is active. Re-implements the reviewed overlay idea from PR #15 with attribution; does not merge #15.
* **Conditional / OR / pick-swap ranges (2026-09-21):** `dynasty_analyze_trade` detects conditional, exclusive-OR, and pick-swap language and returns `PARTIAL` + `NEEDS_CLARIFICATION` with `conditional_or_swap_trade`, plus candidates or `delta_min` / `delta_max`. It does not invent one delta. Normal trades still return one value. No write/submit tools. Re-implements the reviewed range idea from PR #15 with attribution; does not merge #15.
* **Still deferred:** HTTP transport, Docker, OAuth, transactions, FantasyCalc early/mid/late slot picking, multi-user.
