...
* **League-true query (2026-09-21):** FantasyCalc params are derived from `league_format` when they map to documented `numTeams` (`8|10|12|14`), `numQbs` (`1|2`), `ppr` (`0|0.5|1`), and `tep` (omitted / `te+` / `te++`). The default 14-team Superflex full-PPR 0.5 TEP path still sends `tep=te+`. Unsupported combinations do not reuse another query; they stay `PARTIAL` / `UNAVAILABLE` with reasons in `source_notes`. Cache isolation remains schema version + normalized query.
* **Provider-backed picks (2026-09-21):** When the active values payload includes FantasyCalc `PICK` rows, pick `value_sources[].source` is `fantasycalc` for a generic `{season} {ordinal}` match. `config_pick_table` is fallback only and must be labeled as such — never as freshly fetched provider data. Banded early/mid/late matching is not part of this contract.
* **Valuation residuals (2026-09-21):** Supported non-TEP queries send `tep=none`. When a generic PICK row is absent and Early/Mid/Late band rows exist, tools stay `PARTIAL`, omit the single-number field, and put low/high plus band labels in `source_notes` (and `missing_value` when a pick is priced). Static table remains fallback only when neither generic nor band rows exist. No `projected_slot`.
* **CSV overlay (2026-09-21):** Optional `values_overlay_path` CSV keyed by Sleeper player id. Contract source name stays `xlsx`. Overlay wins over FantasyCalc when the file loads. A missing configured file is an explicit status, not a silent FantasyCalc fallback labeled as overlay.
* **Conditional / OR / pick-swap ranges (2026-09-21):** `dynasty_analyze_trade` does not invent one `value_math.delta` for conditional (`if` / `unless` / `when` / `whenever` / `conditional`), exclusive-OR (`or`), or pick-swap (`swap` / `pick swap`) language. Those responses are `PARTIAL` + `NEEDS_CLARIFICATION` with `FlagType.CONDITIONAL_OR_SWAP_TRADE`, plus `asset_resolution[].candidates` and/or `value_math.delta_min` / `delta_max`. Point `delta` / `delta_pct` / package totals stay null on that path. A normal trade still returns one `delta` with `delta_min` / `delta_max` null. No write tools and no trade submission. Additive `ValueMath` fields; `schema_version` stays `"1.0"`.
